...
def find_real_statement(full_text: str, heading_variations: list, window: int = 6000) -> str:
    """
    ...
    NOTE on the old "date nearby" fingerprint: it used to check for phrases
    like "years ended" or "as of september" right after the heading, on the
    theory that a table-of-contents mention wouldn't have those. That broke
    on Apple's 10-K (and likely others): its TOC literally spells out the
    date range in the entry itself, e.g.
        "Consolidated Balance Sheets as of September 27, 2025 and
         September 28, 2024 ... 31"
    which trips the exact same fingerprint as the real table, so the
    function locked onto the TOC line and returned a truncated/wrong
    slice instead of the actual financial data. On Apple's filing this
    silently dropped total_assets, total_liabilities, and the entire cash
    flow statement, which is why every ratio depending on them came back
    "missing data" even though the figures are right there in the PDF.

    Fix: real statement tables in SEC filings are, near-universally,
    immediately followed by a units line like "(In millions)",
    "(in thousands, except per share data)", "(Dollars in millions)", etc.
    Table-of-contents entries never have this - they're just a page
    listing. That's a much more reliable "is this the real table" signal
    than the presence of a date, and it's used as the primary check now.
    A date-based fallback is kept for filers that report in whole dollars
    with no units line, but it now also rejects matches that look like a
    table-of-contents entry (line ends in a bare page number, or another
    core statement heading appears immediately after - a giveaway that
    we're inside a listing of statements rather than a real table).
    """
    unit_marker = re.compile(
        r"\(\s*(?:in|dollars?\s+in|amounts?\s+in|\$\s*in)\s+"
        r"(?:thousands|millions|billions)",
        re.IGNORECASE,
    )
    date_fingerprint = re.compile(
        r"year ended|years ended|as of december|as of june|as of september|"
        r"fiscal year ended|months ended",
        re.IGNORECASE,
    )
    page_number_at_end_of_line = re.compile(r"[^\S\n]\d{1,4}\s*\n")
    other_core_headings = re.compile(
        r"consolidated (?:balance sheet|statements? of (?:income|operations|"
        r"cash flows|shareholders|stockholders))",
        re.IGNORECASE,
    )
    ...
    for heading in heading_variations:
        ...
        while True:
            ...
            if unit_marker.search(chunk[:400]):
                return chunk  # strong signal: this is a real table

            if date_fingerprint.search(chunk[:250]):
                looks_like_toc = bool(
                    page_number_at_end_of_line.search(chunk[:150])
                    or other_core_headings.search(chunk[len(heading):600])
                )
